...
function loadKeysFromEnv() {
  ...
  try {
    ...
    logger.error(
      `[KeyPool] Failed to read env file at ${ENV_PATH}: ${err.message}`,
    );
    return;
  }
...
async function validateGeminiKey(key) {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    timeoutId = setTimeout(
      () => reject(Object.assign(new Error("timeout"), { _isTimeout: true })),
      5000,
    );
  });
  try {
    ...
    if (e._isTimeout) return { status: "unverified", key };
    const msg = e.message || "";
...
async function initialize() {
  ...
  logger.info(
    `Found ${geminiCount} Gemini keys, ${elevenCount} ElevenLabs keys - validating...`,
  );
  ...
  const geminiResults = await processInBatches(
    pool.gemini.keys,
    validateGeminiKey,
  );
  const elevenLabsResults = await processInBatches(
    pool.elevenlabs.keys,
    validateElevenLabsKey,
  );

  const workingGemini = geminiResults
    .filter((r) => r.status === "working")
    .map((r) => r.key);
  const rateLimitedGemini = geminiResults
    .filter((r) => r.status === "rate_limited")
    .map((r) => r.key);
  const unverifiedGemini = geminiResults
  ...
  if (workingGemini.length > 0) {
    ...
    pool.gemini.candidate =
      workingGemini[1] || rateLimitedGemini[0] || unverifiedGemini[0] || null;
    ...
    logger.warn(
      `All Gemini keys rate limited - using ${maskKey(pool.gemini.primary)}`,
    );
  ...
  const workingEleven = elevenLabsResults
    .filter((r) => r.status === "working")
    .map((r) => r.key);
  const rateLimitedEleven = elevenLabsResults
    .filter((r) => r.status === "rate_limited")
    .map((r) => r.key);
  const unverifiedEleven = elevenLabsResults
    .filter((r) => r.status === "unverified")
    .map((r) => r.key);
  const validEleven = [
    ...workingEleven,
  ...
  if (workingEleven.length > 0) {
    ...
    pool.elevenlabs.candidate =
      workingEleven[1] || rateLimitedEleven[0] || unverifiedEleven[0] || null;
  } else if (rateLimitedEleven.length > 0) {
    pool.elevenlabs.primary = rateLimitedEleven[0];
    pool.elevenlabs.candidate =
      rateLimitedEleven[1] || unverifiedEleven[0] || null;
    logger.warn(
      `All ElevenLabs keys rate limited - using ${maskKey(pool.elevenlabs.primary)}`,
    );
    rateLimitedEleven.forEach((key) => {
      pool.elevenlabs.rateLimitedUntil.set(key, Date.now() + 60000);
    });
  ...
  initialized = true;
  logger.info(
    `Key pool ready - Gemini: ${validGemini.length} valid (${workingGemini.length} working), ElevenLabs: ${validEleven.length} valid`,
  );
  return true;
}
...
function invalidate() {
  ...
  pool.gemini.rateLimitedUntil.clear();
  pool.elevenlabs.keys = [];
  pool.elevenlabs.primary = null;
  pool.elevenlabs.candidate = null;
  pool.elevenlabs.rateLimitedUntil.clear();
}
...
module.exports = {
  ...
  getStats,
  isHealthy: () =>
    pool.gemini.keys.length > 0 || pool.elevenlabs.keys.length > 0,
  validateGeminiKey,
  validateElevenLabsKey,
};
